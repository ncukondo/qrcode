...
const reading$1 = writable(false);

const qrdata$$1 = derived(value$1, $value => $value);
const isReading$$1 = derived(reading$1, $reading => $reading);
...
async function startAnalyzing(video) {
  ...
  const jsQRModule = await import('./jsQR-e5228fbf.js');
  ...
  const interval = setInterval(() => {
    ...
    if (code && code.data !== get_store_value(value$1)) {
      console.log("Found QR code", code, code.data);
      value$1.set(code.data);
    }
  }, 500);
}

function startCamera() {
  startReading(navigator.mediaDevices.getUserMedia);
}

async function startReading(mediaFunc) {
  if (get_store_value(reading$1)) stop();
  reading$1.set(true);
...
const file = "src/components/QRCamera.svelte";
...
function create_fragment(ctx) {
	...
	function select_block_type(ctx, dirty) {
		if (/*$isReading$*/ ctx[1]) return create_if_block;
	...
	const block = {
		c: function create() {
			...
			div5 = element("div");
			video = element("video");
			add_location(b_message, file, 18, 10, 335);
			attr_dev(div0, "class", "column");
			add_location(div0, file, 17, 8, 304);
			attr_dev(div1, "class", "columns");
			add_location(div1, file, 16, 6, 274);
			attr_dev(div2, "class", "container");
			add_location(div2, file, 15, 4, 244);
			add_location(section0, file, 14, 2, 230);
			attr_dev(button, "class", "button");
			add_location(button, file, 28, 12, 623);
			attr_dev(div3, "class", "column");
			add_location(div3, file, 27, 10, 590);
			attr_dev(div4, "class", "columns");
			add_location(div4, file, 26, 8, 558);
			attr_dev(video, "width", /*width*/ ctx[2]);
			attr_dev(video, "height", /*height*/ ctx[3]);
			video.autoplay = true;
			add_location(video, file, 35, 12, 864);
			attr_dev(div5, "class", "column");
			add_location(div5, file, 34, 10, 831);
			attr_dev(div6, "class", "columns");
			add_location(div6, file, 33, 8, 799);
			attr_dev(div7, "class", "container");
			add_location(div7, file, 25, 6, 526);
			attr_dev(div8, "class", "hero-body hero-body-hp-main");
			add_location(div8, file, 24, 4, 478);
			attr_dev(section1, "class", "hero is-medium has-text-centered");
			add_location(section1, file, 23, 2, 423);
			add_location(div9, file, 12, 0, 200);
		},
...
function instance($$self, $$props, $$invalidate) {
	let $qrdata$;
	let $isReading$;
	validate_store(qrdata$$1, "qrdata$");
	component_subscribe($$self, qrdata$$1, $$value => $$invalidate(0, $qrdata$ = $$value));
	validate_store(isReading$$1, "isReading$");
	component_subscribe($$self, isReading$$1, $$value => $$invalidate(1, $isReading$ = $$value));
	let width = 500;
	let height = 500;
	...
	$$self.$inject_state = $$props => {
		...
		if ("$qrdata$" in $$props) qrdata$$1.set($qrdata$ = $$props.$qrdata$);
		if ("$isReading$" in $$props) isReading$$1.set($isReading$ = $$props.$isReading$);
	};
...
function create_fragment$1(ctx) {
	let main;
	let current;
	const qrcamera = new QRCamera({ $$inline: true });

	const block = {
		c: function create() {
			main = element("main");
			create_component(qrcamera.$$.fragment);
			attr_dev(main, "class", "overflow-hidden");
			add_location(main, file$1, 10, 0, 296);
		},
		l: function claim(nodes) {
			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
		},
		m: function mount(target, anchor) {
			...
			mount_component(qrcamera, main, null);
			current = true;
		},
		p: noop,
		i: function intro(local) {
			if (current) return;
			transition_in(qrcamera.$$.fragment, local);
			current = true;
		},
		o: function outro(local) {
			transition_out(qrcamera.$$.fragment, local);
			current = false;
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(main);
			destroy_component(qrcamera);
		}
	};
...
//# sourceMappingURL=main-512bbef1.js.map
